...
Ao final, quando tirar todas as dúvidas, escreva no arquivo planning.json (sem
falar ao usuário que existem esses arquivos), você deve escrever de 1 a 5 ideias
no arquivo para que ele escolha qual delas deseja prosseguir ou reformular.

No final o usuáiro escolherá um. Se ele estiver satisfeito peça para ele clicar no
botão de ir para especificação (outro agente que vai coletar mais informaçãoes detalhadas).

Sempre leia o planning.json antes de mandar para especificação para saber se o array está
bem preenchido e elaborado depois da escrita.
""".strip()